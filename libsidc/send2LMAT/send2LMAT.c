...
static gboolean lmat_spawn(gchar ** argv, gboolean synchronous) {
    ...
    if (synchronous)
        ret = g_spawn_sync(NULL, argv, NULL, flags, NULL, NULL, NULL, NULL, NULL, &err);
    else {
#ifdef G_OS_UNIX
        struct sigaction sa;
...
static gboolean p2sc_send2LMAT(char *nam, char *ver, char *rid, char *msg,
                               char *mid, char *loc, char *dbg, char *pid, char *host) {
    gboolean ret = FALSE;
    gchar *send2LMATscript = g_build_filename(SEND2LMAT_DIR, "send2LMAT.pl", NULL);
